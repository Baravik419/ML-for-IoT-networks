...
from data_preprocessing_integration import generate_folds

def train_model(use_smote=True, chosen_fold_number=None):

    start_total = time.perf_counter()

    folds, Label_Encoder, Ordinal_Encoder, X_columns = generate_folds(use_smote=use_smote, chosen_fold_number=chosen_fold_number)
    ...
    for fold in folds_to_run:
        ...
        XGBoost_Model.fit(X_train, y_train) # Starting to learn
...
if __name__ == "__main__":
    ...
    # Exporting
    os.makedirs("XGBoost_Model_Integration (5 fold)", exist_ok=True)

    joblib.dump(results["best_model"], "XGBoost_Model_Integration (5 fold)/XGBoost_Model_Integration.pkl")
    joblib.dump(results["best_scaler"], "XGBoost_Model_Integration (5 fold)/XGB_Integration_scaler.pkl")
    joblib.dump(results["Label_Encoder"], "XGBoost_Model_Integration (5 fold)/XGB_Integration_Label_Encoder.pkl")
    joblib.dump(results["Ordinal_Encoder"], "XGBoost_Model_Integration (5 fold)/XGB_Integration_Ordinal_Encoder.pkl")
    joblib.dump(results["X_columns"], "XGBoost_Model_Integration (5 fold)/XGB_Integration_X_columns.pkl")
    ...
    probability_df.to_csv("XGBoost_Model_Integration (5 fold)/XGB_Integration_probabilities.csv", index=False)

    with open("XGBoost_Model_Integration (5 fold)/XGB_Integration_metrics.txt", "w", encoding="utf-8") as f:
        f.write("\n".join(report_text))
